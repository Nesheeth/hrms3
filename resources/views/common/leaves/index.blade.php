					...
					<div class="panel-body">
					    <div class="row">  
					       <div class="col-sm-12">
					       	  ...
					       	  <div class="col-sm-4">  <h3><span class="label label-danger"> Rejected Leave : {{$rejected}} </span></h3>  </div>
					       </div>
					       <div class="col-sm-9">
							...
							<div class="col-sm-3" >
		    		   	         <a href="{{route('cm.apply_leave',['role'=> getRoleStr()])}}" class="btn btn-primary pull-right" {{ (Auth::user()->is_active==1)?'':'disabled' }}> Apply Leave</a> 
							</div>
								...
								<tbody>
									...
									<tr>
										...
										<td> 
										    ...
										    @endif
										    @if($leave->status == 2 && date('Y-m-d', strtotime($leave->date_from)) >= date('Y-m-d')) 
											   <button class="btn btn-warning re_apply" rel="{{$leave->id}}" type="button"> Re-Apply </button> 
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(document).on('change','#sort_by',function(){ 
			 window.location.href = "{{route('cm.leave_mng',['role'=>getRoleStr()])}}"+"?&leave_type="+ $(this).val();
		});
	...
	$('.re_apply').click(function(){
		...
		swal({
			  title: "Are you sure?",
			  text: "Do you want to apply again for this leave.",
			  type: "warning",
			  buttons: true,
			  dangerMode: true,
			})
			.then((willDelete) => {
              ...
              var data ={};
               ...
               $.ajax({
                 url:"{{url(getRoleStr().'/re-apply-leave')}}",
                 method:'get',
                 dataType:'json',
                 data:data,
                 success:function(result){
                 	if(result.status){
                 		swal('Success','Leave re-apply successfully','success');
                 		location.reload();
                 	}
