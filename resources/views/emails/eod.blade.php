                           ...
                           <table cellpadding="0" cellspacing="0" class="force-full-width">
                              <tr>
                                 <td height="70" valign="top" class="header-cell">  
                                    <img width="196" height="60" src="{{asset('public/images/cb-logo.png')}}"  alt="logo" style="margin:15px;">
                                 </td>
                                 <td height="70" valign="top" style="background: #0d85eef2;
                                    padding: 15px;
                                    text-align: right;
                                    color: #fff" >
                                    <b> Date: </b> {{ date('d M - Y', strtotime($date_of_eod)) }}
                                 </td>
                              </tr>
                              <tr>
                                    ...
                                    <table cellpadding="0" cellspacing="0" width="100%" bgcolor="#ffffff">
                                       ...
                                       <tr>
                                          <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
                                             Here is my today’s work status 
                                          </td>
                                       </tr>
                                       <tr>
                                          <td>
                                             <table cellspacing="1" cellpadding="1" width="100%" bgcolor="#ffffff" BORDER="1" BORDERCOLORLIGHT="#C7F25F" >
                                                <thead align="left" style="background-color:#0f81e4; color:#fff; text-align:center;">
                                                   <tr>
                                                      ...
                                                      <th style="width:10%;">ES Hours</th>
                                                      <th style="width:5%;">Hours Spent Today</th>
                                                      <th style="width:5%;">Total Hour</th>
                                                      <th style="width:12%;">Delivery Date</th>
                                                      <th style="width:6%;">Task Status</th>
                                                   </tr>
                                                </thead>
                                                <tbody style="background-color:#1c87e41f;">
                                                   @foreach($eods as $eod)
                                                   <tr>
                                                      <th style="width:12%;"> {{ date('d M - Y ', strtotime($eod->date)) }} </th>
                                                      <th style="width:10%;"> {{ $eod->project->project_name }} </th>
                                                      <th style="width:10%;"> {{ $eod->task_name }} </th>
                                                      <th style="width:30%;"> {{ $eod->description }} </th>
                                                      <th style="width:10%;"> {{ $eod->es_hours }} </th>
                                                      <th style="width:5%;">  {{ $eod->today_hours }} </th>
                                                      <th style="width:5%;">  {{ $eod->total_hours }} </th>
                                                      <th style="width:12%;"> {{ date('d M - Y ', strtotime($eod->delivery_date)) }}</th>
                                                      <th style="width:6%;">  {{ $eod->task_status }} </th>
                                                   </tr>
                                                   @endforeach
                                                </tbody>
                                             </table>
