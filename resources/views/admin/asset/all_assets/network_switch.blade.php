	...
	<div class="page-title">
		...
		<div class="page-breadcrumb">
			<ol class="breadcrumb">
				...
				<li><a href="{{URL('admin/assets/all_assets')}}">All Assets</a></li>
				<li class="active">Network switch</li>
			</ol>
		</div>
	</div>
	<div id="main-wrapper">
			...
			<div class="col-md-12">
				...
				<div class="panel panel-white">
					<div class="panel-heading clearfix">
						<a href="{{URL('/admin/export/asset-sheet-network_switch')}}" class="btn btn-danger pull-right" style="margin-top: -16px;margin-right: 10px;">Export Assets</a>
							...
							<table class="table"  id="datatable">
								...
								<tbody>
									...
									<tr>
										...
										<td><b>{{is_null($asset->system_code)?"N/A":$asset->system_code}}</b></td>
										<td>{{$asset->status}}</td>
										<td><a class="btn btn-primary" href="{{URL('/admin/network_switch/'.$asset->id)}}">Edit</a></td>
										<td>{{getUserById($asset->last_editing_by)->first_name}} {{getUserById($asset->last_editing_by)->last_name}}</td>
...
<script>
	...
	function releaseAsset(asset_id){
		var url = "{{URL('admin/assetRelease/')}}";
	...
	$('#uploadAsset').on('click',function(){
		...
		if(assetFile.files[0]) {
			...
			var url  = "{{URL('/admin/import_network_switch/asset-sheet')}}";
			$.ajax({
				url: url,
				type: 'POST',
				data: form_data,
