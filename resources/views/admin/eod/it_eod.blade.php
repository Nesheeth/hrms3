								...
								<tbody>
									...
									<tr>
										...
										<td> {{$eod->user->first_name}} {{$eod->user->last_name}} </td>  
											{{--<td> {{$eod->task_id}}</td>--}}   
										<td> {{$eod->issue_type}}</td>   
										<td> {{$eod->cbpc_no}}</td>   
										<td> {{$eod->issue_details}}</td>   
										<td> {{$eod->resolution_provided}}</td>   
										<td> {{$eod->resolution_status}}</td>   
											{{--<td> {{$eod->comment}}</td> --}}
										<td>
											<button class="btn btn-info btn-sm eod_details" rel="{{ $eod->id }}">  See Details </button> 
											<!-- <a onclick="return confirm('Are you sure, You want to delete EOD ?')" class="btn btn-danger btn-sm" href="{{route('admin.it_eod_delete',['eod_id'=> $eod->id ])}}">  Delete </a> -->
										</td> 
									</tr>
									@endforeach
									</tbody>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">IT Executive EOD Details</h4>
      </div>
	...
	$(document).ready(function(){ 
		 $(document).on('click','.eod_details',function(){  
		 	var id = $(this).attr('rel');
		 	    $.ajax({
		 	    	url:"{{route('admin.it_eod_details')}}", 
		 	    	type:"GET",
		 	    	data:{id:id},
