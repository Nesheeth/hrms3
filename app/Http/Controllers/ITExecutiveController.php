...
class ITExecutiveController extends Controller{
	...
	public function dashboard(){
		...
		return view('itExecutive.dashboard',$data);
	} 
...

	{
		...
		if(is_null($asset)){
			...
			$data['master_asset'] = \App\MasterAsset::first();
...

	{
		...
		return view('itExecutive.holidays.calender',$data);
	...
	/*echo "<pre>";
	print_r($systemExcel->toArray()); 
	echo "</pre>";
	die;*/
	try{
	   	return Excel::create('Desktop Lists', function($excel) use($systemExcel) {
			$excel->sheet('Sheet 1', function($sheet) use($systemExcel) {
				$sheet->fromArray($systemExcel);
			}); 
		})->export('xls'); 
   ...
   public function importAssetExcel_almirah(Request $request)
	...
	public function postProfileEdit(Request $request){
		...
		$validate = $this->validate($request,[
			'first_name'=>'required',
			'dob'=>'required|date|before_or_equal:'.date('d/m/Y',strtotime(date('d/m/Y')))
		]);
...
public function calender(Request $request){
        ...
        return view('itExecutive.holidays.calenderdata', $data);
    }
